-- =============================================================================
--                  Well Management System - Database Schema (v3 - Idempotent)
--
-- This script is now idempotent, meaning it can be run multiple times without
-- causing errors. It will first drop all existing objects (tables, types) in the
-- correct order before recreating them. This is ideal for development.
--
-- =============================================================================

-- Step 0: Drop Existing Objects in Reverse Order of Dependency
-- -----------------------------------------------------------------
-- This ensures a clean slate every time the script is run. We use IF EXISTS
-- so that the script doesn't fail on its very first run when nothing exists yet.
...
-- Step 2: Create Custom Data Types (ENUMs)
...
CREATE TABLE areas (
    ...
    -- CORRECTED TYPE: Using GEOMETRY instead of GEOGRAPHY
